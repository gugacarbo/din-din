...
async function bootstrap(db: Database, userId: string) {
...
export const listCategories = createServerFn({ method: "GET" })
	.validator(
		z.object({ status: z.enum(["active", "archived"]).default("active") }),
	)
	.handler(async ({ data }) => {
		const { db, userId } = await context();
		await bootstrap(db, userId);
		...
		const rows = await db
			.select()
			.from(categories)
			.where(and(eq(categories.userId, userId), archivedFilter))
...
export const createCategory = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		if (existing[0]) {
			...
			return categoryDto(await ownedCategory(db, userId, existing[0].id));
		}
		...
		return categoryDto(await ownedCategory(db, userId, id));
	});

export const updateCategory = createServerFn({ method: "POST" })
	.validator(
		categoryInput.omit({ type: true }).extend({ id: z.string().uuid() }),
	)
	.handler(async ({ data }) => {
		const { db, userId } = await context();
		const previous = await ownedCategory(db, userId, data.id);
		const normalizedName = normalizeCategoryName(data.name);
		...
		await db
			...
			.where(
				and(eq(categories.id, previous.id), eq(categories.userId, userId)),
			);
		return categoryDto(await ownedCategory(db, userId, previous.id));
	});
...
export const archiveCategory = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		const { db, userId } = await context();
		await ownedCategory(db, userId, data.id);
		await db
			.update(categories)
...
export const createTransaction = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		return transactionDto(db, userId, id);
	});
export const updateTransaction = createServerFn({ method: "POST" })
	.validator(transactionInput.extend({ id: z.string().uuid() }))
	.handler(async ({ data }) => {
		const { db, userId } = await context();
		...
		await db
			...
			.where(
				and(eq(transactions.id, data.id), eq(transactions.userId, userId)),
			);
		return transactionDto(db, userId, data.id);
	});
...
export const listTransactions = createServerFn({ method: "GET" })
	...
	.handler(async ({ data }) => {
		...
		const rows = await db
			...
			.from(transactions)
			.where(and(eq(transactions.userId, userId), archivedFilter, cursorFilter))
			.orderBy(
		...
		return {
			items: await Promise.all(
				page.map((row) => transactionDto(db, userId, row.id)),
			),
			nextCursor:
...
export const getDashboard = createServerFn({ method: "GET" }).handler(
	async () => {
		...
		const recentIds = await db
			...
			.from(transactions)
			.where(
				and(eq(transactions.userId, userId), isNull(transactions.archivedAt)),
			)
			.orderBy(
				desc(transactions.occurredAt),
				desc(transactions.createdAt),
		...
		return {
			...
			recentTransactions: await Promise.all(
				recentIds.map((row) => transactionDto(db, userId, row.id)),
			),
		};
	},
);
