...
implemented-by:
  ...
  - src/lib/admin-invite-client.ts
  - src/routes/__root.tsx
  - src/routes/admin/convite.tsx
...
- Convites administrativos expiram em 24h; o token cru chega apenas no fragmento inicial. Antes do router, o bootstrap o copia para `sessionStorage` da mesma origem/aba e remove o fragmento com `history.replaceState`; renderizações posteriores leem essa cópia efêmera, nunca a URL. O cliente a apaga imediatamente depois de `POST /api/admin/invite/prepare` bem-sucedido; em falha mantém-na somente para nova tentativa na mesma aba. O servidor persiste somente HMACs e uma continuação opaca HttpOnly/Secure/SameSite. `APP_SECRET` é exclusivo deste fluxo e não reutiliza o segredo Better Auth.
...
| 8   | renderização repetida antes de preparar o convite | conservar token somente em `sessionStorage` da aba e enviar no POST sem token na URL   |
| 9   | `prepare` falha ou conclui com sucesso            | manter o token para retry na falha; removê-lo imediatamente no sucesso, antes do OAuth |
...
- Cobrir em teste UI o bootstrap do fragmento, renderização repetida, POST inicial com token e remoção de `sessionStorage` somente após sucesso; cobrir retorno OAuth sem fragmento usando a continuação existente nos testes Workers.
