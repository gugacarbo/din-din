...
--> statement-breakpoint
CREATE UNIQUE INDEX `categories_user_type_parent_name_unique` ON `categories__next` (`user_id`,`type`,coalesce(`parent_category_id`, '__root__'),`normalized_name`);
--> statement-breakpoint
CREATE TABLE `payment_methods` (
	...
	`name` text NOT NULL,
	`kind` text NOT NULL,
	`invoice_control` integer NOT NULL DEFAULT 0,
	`closing_day` integer,
	`due_day` integer,
	`archived_at` integer,
	`created_at` integer NOT NULL,
...
CREATE TABLE `transactions__next` (
	...
	CONSTRAINT `transactions_payment_method_owner_fk` FOREIGN KEY (`payment_method_id`,`user_id`) REFERENCES `payment_methods`(`id`,`user_id`),
...
SELECT `id`,`user_id`,`type`,`name`,`normalized_name`,`color_key`,`icon_key`,NULL,`archived_at`,`created_at`,`updated_at` FROM `categories`;
--> statement-breakpoint
...
SELECT `id`,`user_id`,`category_id`,NULL,`type`,`amount_cents`,`currency`,`occurred_at`,`description`,NULL,NULL,`archived_at`,`created_at`,`updated_at` FROM `transactions`;
--> statement-breakpoint
