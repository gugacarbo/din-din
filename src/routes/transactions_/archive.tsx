...
import { transactionsQueryOptions } from "#/lib/finance-query-options.ts";
import { requireFinanceSession } from "#/lib/route-session.ts";

export const Route = createFileRoute("/transactions_/archive")({
	beforeLoad: ({ context }) => requireFinanceSession(context.queryClient),
	loader: ({ context }) =>
		context.queryClient.ensureInfiniteQueryData(
			transactionsQueryOptions("archived"),
		),
